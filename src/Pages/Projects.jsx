  ...
  return (
    <article className="flex flex-col border rounded-lg border-gray-700 text-secondary bg-primary overflow-hidden h-full">
      ...
      <div className="bg-gray-800 px-3 py-2 flex items-center justify-end gap-1 border-b border-gray-700">
        ...
        <button className="w-5 h-5 flex items-center justify-center hover:bg-red-600 transition-colors rounded">
          <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 16 16">
            <path
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {techstack.map((t, i) => (
            <span
              key={i}
              className="px-2 py-1 text-xs sm:text-sm rounded-md border border-gray-700"
            >
              {t}
            </span>
          ))}
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex items-start justify-between gap-4 bg-primary p-4">
              ...
              <ModalBody className="bg-primary p-4 space-y-3 text-xs sm:text-sm md:text-md lg:text-lg">
                {Array.isArray(details) ? (
                  details.map((d, idx) => <p key={idx}>{d}</p>)
                ) : (
                  <p>{details}</p>
                )}
              </ModalBody>

              <ModalFooter className="bg-primary p-4"></ModalFooter>
            </>
          )}
        </ModalContent>
      </Modal>
...
const Projects = ({ data = [] }) => {
  ...
  return (
    <main className="min-h-screen bg-primary  py-12 md:py-24">
